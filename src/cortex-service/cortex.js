        ...
        try {
          if (JSON.parse(data)["id"] == QUERY_HEADSET_ID) {
            ...
            if (JSON.parse(data)["result"].length > 0) {
              ...
              console.log("No headset detected.");
              reject({
                message: "No headset detected",
              });
              this.isHeadsetConnected = false;
            }
          }
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        try {
          ...
          } else if (JSON.parse(data)["id"] == AUTHORIZE_ID) {
        ...
        try {
          if (JSON.parse(data)["id"] == CREATE_RECORD_REQUEST_ID) {
            ...
            if (parsedData.error) {
              ...
              let recordId = parsedData["result"]["record"]["uuid"];
    ...
    this.socket.on("open", async () => {
      ...
      for (let trainingAction of trainingActions) {
        for (let numTrain = 0; numTrain < numberOfTrain; numTrain++) {
          ...
          console.log(
            `START TRAINING "${trainingAction}" TIME ${
              numTrain + 1
            } ---------------`
          );
          console.log("\r\n");
