...
import kornia
from generative.transformer.XTransformer import Encoder, TransformerWrapper
...
class SpatialRescaler(nn.Module):
    def __init__(self,
        ...
        if self.remap_output:
            print(f'Spatial Rescaler mapping from {in_channels} to {out_channels} channels after resizing.', flush=True)
...
class EnhancedImageEmbedder(nn.Module):
    def __init__(self, frozen_embedder, output_dim=768):
        super().__init__()
    ...
    def forward(self, inputs):
        with torch.no_grad():
            image_embeds = self.frozen_embedder(inputs)
        enhanced_embeds = self.additional_layers(image_embeds)
...
#     )
    def contrastive_loss(logits, dim):
        ...
        print(m, flush=True)
        neg_ce = torch.diag(m)
        print(neg_ce, flush=True)
        print(-neg_ce.mean(), flush=True)
        return -neg_ce.mean()
...
#     print(clip_loss(logits_per_text))
    ...
    print(similarity_matrix, flush=True)
    print(clip_loss(similarity_matrix), flush=True)
