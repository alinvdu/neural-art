...
from generative.ldm.LdmUtil import log_txt_as_img, exists, default, ismap, isimage, mean_flat, count_params, instantiate_from_config
from generative.distributions.Distributions import normal_kl, DiagonalGaussianDistribution
from generative.autoencoders.Autoencoders import VQModelInterface, IdentityFirstStage, AutoencoderKL
from generative.diffusion.DiffusionUtil import make_beta_schedule, extract_into_tensor, noise_like
from generative.diffusion.PlmSampler import PLMSSampler
from generative.encoders.EncodersModules import FrozenImageEmbedder, EnhancedImageEmbedder
...
from generative.litema.LitEma import LitEma
from torch import nn
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        print(f'Loss Type is: {loss_type}', flush=True)
        super().__init__()
        assert parameterization in ["eps", "x0"], 'currently only supporting "eps" and "x0"'
        self.parameterization = parameterization
        print(f"{self.__class__.__name__}: Running in {self.parameterization}-prediction mode", flush=True)
        ...
        if self.use_ema:
            ...
            print(f"Keeping EMAs of {len(list(self.model_ema.buffers()))}.", flush=True)
    ...
    def ema_scope(self, context=None):
        if self.use_ema:
            ...
            if context is not None:
                print(f"{context}: Switched to EMA weights", flush=True)
        try:
            yield None
        finally:
            if self.use_ema:
                ...
                if context is not None:
                    print(f"{context}: Restored training weights", flush=True)
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        for k in keys:
            for ik in ignore_keys:
                if k.startswith(ik):
                    print("Deleting key {} from state_dict.".format(k), flush=True)
                    del sd[k]
        missing, unexpected = self.load_state_dict(sd, strict=False) if not only_model else self.model.load_state_dict(
            sd, strict=False)
        print(f"Restored from {path} with {len(missing)} missing and {len(unexpected)} unexpected keys", flush=True)
        if len(missing) > 0:
    ...
    def log_losses_progression(self, loss_dict, output_path):
        try:
            ...
            # Log the exception
            print(f"Error occurred in log_losses_progression: {e}", flush=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if not self.outputImageForEpoch.get(self.trainer.current_epoch, False) and ((self.trainer.current_epoch + 1) % 5 == 0):
        #if not self.outputImageForEpoch.get(self.trainer.current_epoch, False) and (self.trainer.current_epoch + 1 == 2):
            print(self.trainer.current_epoch, flush=True)
            self.outputImageForEpoch[self.trainer.current_epoch] = True
            # Generate images from the validation batch
            grid_val, _, _ = self.generate(batch, ddim_steps=167, num_samples=3, limit=3)
            grid_val_imgs = Image.fromarray(grid_val.astype(np.uint8))
            grid_val_imgs.save(os.path.join(self.output_path, f'samples_val-{self.trainer.current_epoch}.png'))

            # Get a random batch from the training dataloader
            train_dataloader = self.dataloader
            train_batch = random.choice([b for b in train_dataloader])

            # Generate images from the random training batch
            grid_train, _, _ = self.generate(train_batch, ddim_steps=167, num_samples=3, limit=3)
            grid_train_imgs = Image.fromarray(grid_train.astype(np.uint8))
            grid_train_imgs.save(os.path.join(self.output_path, f'samples_train-{self.trainer.current_epoch}.png'))
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        if self.clip_tune:
            ...
            self.image_embedder = EnhancedImageEmbedder(frozen_image_embedder)
        self.cls_tune = False
        self.temperature = temperature
    ...
    def on_train_batch_start(self, batch, batch_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            print("### USING STD-RESCALING ###", flush=True)
            ...
            print(f"setting self.scale_factor to {self.scale_factor}", flush=True)
            print("### USING STD-RESCALING ###", flush=True)
    ...
    def instantiate_cond_stage(self, config):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                print("Using first stage also as cond stage.", flush=True)
                self.cond_stage_model = self.first_stage_model
            elif config == "__is_unconditional__":
                print(f"Training {self.__class__.__name__} as an unconditional model.", flush=True)
                self.cond_stage_model = None
        ...
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ...
                if ks[0] > h or ks[1] > w:
                    ...
                    print("reducing Kernel", flush=True)
                print("print 3", flush=True)
                if stride[0] > h or stride[1] > w:
                    stride = (min(stride[0], h), min(stride[1], w))
                    print("reducing stride", flush=True)

                fold, unfold, normalization, weighting = self.get_fold_unfold(z, ks, stride, uf=uf)
                print("print 4", flush=True)
                z = unfold(z)  # (bn, nc * prod(**ks), L)
                # 1. Reshape to img shape
                z = z.view((z.shape[0], -1, ks[0], ks[1], z.shape[-1]))  # (bn, nc, ks[0], ks[1], L )
                print("print 5", flush=True)
                ...
                if isinstance(self.first_stage_model, VQModelInterface):
                    ...
                    print("print 6", flush=True)
                    output_list = [self.first_stage_model.decode(z[:, :, :, :, i])
                                   for i in range(z.shape[-1])]
                print("print 7", flush=True)
                o = torch.stack(output_list, axis=-1)  # # (bn, nc, ks[0], ks[1], L)
                o = o * weighting
                print("print 8", flush=True)
                # Reverse 1. reshape to img shape
                o = o.view((o.shape[0], -1, o.shape[-1]))  # (bn, nc * ks[0] * ks[1], L)
                # stitch crops together
                decoded = fold(o)
                print("print 9", flush=True)
                decoded = decoded / normalization  # norm is shape (1, 1, h, w)
                print("print 10", flush=True)
                return decoded
            else:
                if isinstance(self.first_stage_model, VQModelInterface):
                    print("print 11", flush=True)
                    return self.first_stage_model.decode(z, force_not_quantize=predict_cids or force_not_quantize)
                else:
                    print("print 12", flush=True)
                    return self.first_stage_model.decode(z)

        else:
            print("print 13", flush=True)
            if isinstance(self.first_stage_model, VQModelInterface):
                print("print 14", flush=True)
                return self.first_stage_model.decode(z, force_not_quantize=predict_cids or force_not_quantize)
            else:
                print("print 15", flush=True)
                return self.first_stage_model.decode(z)
    ...
    def on_epoch_end_m(self, currentEpoch):
        if currentEpoch in self.loss_progression_epochs:
            ...
            print(f'loss end, here are the losses, loss: {mean_loss}, clip_loss: {mean_loss_clip}', flush=True)
    ...
    def p_sample(self, x, c, t, clip_denoised=False, repeat_noise=False,
        ...
        print(f'p_sample: ${temperature}', flush=True)
    ...
    def configure_optimizers(self):
        ...
        if self.train_cond_stage_only:
            print(f"{self.__class__.__name__}: Only optimizing conditioner params!", flush=True)
            ...
            params = list(self.model.parameters())
            if self.cond_stage_trainable:
                print(f"{self.__class__.__name__}: Also optimizing conditioner params!", flush=True)
                params = params + list(self.cond_stage_model.parameters())
            if self.learn_logvar:
                print('Diffusion model optimizing logvar', flush=True)
        ...
        if self.use_scheduler:
            ...
            print("Setting up LambdaLR scheduler...", flush=True)
            scheduler = [
                {
